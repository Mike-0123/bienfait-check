...
const JoinCard = ({ title, description, img }: { img: string, description: string, title: string }) => {
    return (
        <div className={'bg-bgColor text-textColor mx-auto w-full flex flex-col justify-between shadow-2xl shadow-middleColor lg:flex-row rounded-lg'}>
            ...
            <div className="bg-white p-5 lg:w-1/2 lg:pt-20 lg:px-10 rounded-lg flex flex-col gap-5 items-center justify-start">
                ...
                <Link to='/about' className="uppercase bg-textColor px-5 py-3 text-bgColor text-xs rounded-md hover:bg-shineColor">BECOME ASSOCIATE</Link>
            </div>
        </div>
    )
}
