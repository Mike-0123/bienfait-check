...
import { api } from '../Api'
import { commonStyleClasses, isEmailValid } from '../components/utils'
...
export const DashboardLogin = () => {
  ...
  async function handleLogin(e) {
    ...
    setLoggingIn(true)
    try {
      const { status, resStatus, resMsg } = await api.adminLoginAPI(loginData.email, loginData.password);
      if (resStatus) {
        ...
        document.cookie = `${dashboardInfo.tokenName}=${resMsg.adminToken}; path=/; expires=${expiryDate}`;
        loginAdmin(resMsg.admin)
